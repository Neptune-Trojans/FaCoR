...
def baseline_model2(model_path, device):
...
def final_statistics(args):
    ...
    model = baseline_model2(model_path, device)
    classes = [
    ...
    with torch.no_grad():
        ...
        for img1, img2, labels, classes, batch_list in tqdm(gen(test_samples, batch_size)):
            if img1 is not None:

                # img1, img2, labels, classes, batch_list = data[1]
                img1 = torch.from_numpy(img1).type(torch.float).to(device=device)
                img2 = torch.from_numpy(img2).type(torch.float).to(device=device)
